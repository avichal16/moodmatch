const TMDB_API_KEY = "c5bb9a766bdc90fcc8f7293f6cd9c26a"; // Replace with your TMDB key
...
// Generate Tag & Emoji buttons (only if containers exist)
...
// Search for Movies & Books (on index.html)
if (searchInput) {
...
// Fetch recommendations (on index.html)
...
// Watchlist (on watchlist.html)
if (watchlistContainer) {
  const items = getData("watchlist");
  if (!items.length) {
    watchlistContainer.innerHTML = "<p class='p-4 text-gray-500'>No saved titles yet.</p>";
  }
  items.forEach(item=>{
    const card = document.createElement("div");
    card.className = "p-4 border rounded-lg bg-white max-w-[200px]";
